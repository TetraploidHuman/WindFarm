...
- 走廊：只生成不差于直线的候选，终选需模型能量明确更省  
  - 硬地板 `CORRIDOR_HARD_FLOOR_MPS=1.20`（四川级静风默认禁）  
  ...
  - **硬地板以下几何解锁**：无风模型能量仍比同带直线省 ≥3%（或两段 via DEM 减爬 ≥10 m）才放行；四川有风假省电、无风不省 → 仍挡
...
- **防过拟合**：不按地图写特例。曾试「沿航迹顺风边」解锁台湾，`path_model` 变差已撤回；现用通用地形/几何门控

### 最新 8 场景结果（`multi-scenario-energy-20260809-212703`）

| 分组 | vs 名义直线 | vs 最佳高度带 |
|------|-------------|---------------|
| CORE（闽/京/青/辽） | **+6.9%** | **+2.0%** |
| HOLDOUT（疆/川/晋/台） | **+3.7%** | **+3.7%** |
| 全部 8 场景 | **+5.3%** | **+2.8%** |
...
| sichuan_foothills | **0.0** | **0.0** | 硬地板 + 无风几何门仍挡 |
| shanxi_loess | **+6.7** | **+6.7** | 走廊正样本 |
| taiwan_hills | **+8.2** | **+8.2** | 几何走廊兑现（非 |风速| 剪切） |

对比：台湾 **+0.7% → +2.1% → +8.2%**；HOLDOUT **+2.2% → +3.7%**；8 场景均值 **+4.5% → +5.3%**。

### 负收益 / 权衡（当前）

| 类型 | 状态 | 说明 |
|------|------|------|
| A. 弱风误走廊 | **已收敛** | 硬地板 + 无风几何门；四川保持 0 |
| B. 高度带未对齐 oracle | **已收敛** | 青海贴合 `agl_2.667`（`save_best≈0`） |
| C. 台湾弱风走廊 | **已收敛（几何）** | 主因是 AGL/DEM 捷径而非 |风速| 边；沿航迹顺风解锁仍不采用 |
