...
### 最新 16 场景结果（`multi-scenario-energy-20260810-162059`）

| 分组 | vs 名义直线 | vs 最佳高度带 |
|------|-------------|---------------|
| CORE（闽/京/青/辽） | **+10.1%** | **+5.2%** |
| HOLDOUT（12 图） | **+3.3%** | **+2.8%** |
| 全部 16 场景 | **+5.0%** | **+3.4%** |

相对 `152206`（+4.8 / +3.2）：全场 **+0.2**，CORE **+0.4**。闽 **+9.8→+11.2**、晋 **+7.0→+8.1**；台/川/辽/青持平。  
手段续：soft prior 用**真值 ambient**（信念早期风速≈0 不再误杀）+ 真值 Joules 重排；**保留 DEM 爬升预筛**（放开曾使吉林 live −2%）；soft 另需**真值侧向边**（挡住甘肃假 prior）。

| 场景 | save_agl% | save_best% | 备注 |
|------|-----------|------------|------|
| fujian_hills | **+11.2** | **+11.2** | soft prior 再抬 |
...
| shanxi_loess | **+8.1** | **+8.1** | soft+真值边（原 +7.0） |
