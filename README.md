...
### 最新 8 场景结果（`multi-scenario-energy-20260809-222757`）

| 分组 | vs 名义直线 | vs 最佳高度带 |
|------|-------------|---------------|
| CORE（闽/京/青/辽） | **+6.9%** | **+2.0%** |
| HOLDOUT（疆/川/晋/台） | **+4.2%** | **+4.2%** |
| 全部 8 场景 | **+5.5%** | **+3.1%** |

| 场景 | save_agl% | save_best% | 备注 |
|------|-----------|------------|------|
| fujian_hills | +0.5 | +0.5 | 直线上已有抬升/顶风；侧向走廊真值无 Er&lt;0.999 |
...
| taiwan_hills | **+8.2** | **+8.2** | 几何走廊兑现（微风水平速门已就绪） |

对比：微风水平速 + 远偏置落地后 8 图均值持平；曾试高带强制爬升时台湾 **+8.2→+3.3** 已撤回。
...
1. **盘旋热利用仍弱**（闽等路径上已有抬升，但侧向走廊无真值收益；需在 guide/MPC 内吃局部热，而不是再加走廊门）
2. 微风走廊仍依赖信念边质量；高带微风爬升与沿航迹顺风门均已试过并撤回  
### 评测怎么读
