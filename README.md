...
### 最新 16 场景结果（`multi-scenario-energy-20260810-031458`）

| 分组 | vs 名义直线 | vs 最佳高度带 |
|------|-------------|---------------|
| CORE（闽/京/青/辽） | **+9.2%** | **+4.3%** |
| HOLDOUT（12 图） | **+2.8%** | **+2.3%** |
| 全部 16 场景 | **+4.4%** | **+2.8%** |

相对扩编首评 `232911`：全场 **+3.6→+4.4**，holdout **+1.7→+2.8**；CORE 持平。  
多 via + 信念边加强后与 `002107` **持平**；开环 2-via 无一图优选；晋/吉 `gap_stf` 仍约 **+2.6 / +3.2**（真值 1-via 更省，live 信念未吃满）。

| 场景 | save_agl% | save_best% | 备注 |
|------|-----------|------------|------|
| fujian_hills | **+9.6** | **+9.6** | STF 能量门后仍满收益 |
| beijing_plain | 0.0 | 0.0 | 平地对照 |
| qinghai_ridge | **+19.6** | **+0.0** | 微爬对齐 2.667 |
| liaoning_coast | **+7.4** | **+7.4** | 走廊正样本 |
| xinjiang_gobi | 0.0 | 0.0 | 干旱开阔 |
| sichuan_foothills | **+5.4** | **+5.4** | 微风走廊 + 轻爬（原 +1.8） |
| shanxi_loess | **+6.7** | **+6.7** | 走廊正样本 |
| taiwan_hills | **+11.6** | **+11.6** | 几何走廊 + 高度（原 +8.5） |
...
1. 若还要抠晋/吉：加侧向探测观测或放宽「信念近平局时」的走廊终选，而不是再加 via  
2. 否则转向真机噪声 / 日志回放；ML 仍低优先
