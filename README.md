...
### 最新 16 场景结果（`multi-scenario-energy-20260810-141422`）

| 分组 | vs 名义直线 | vs 最佳高度带 |
|------|-------------|---------------|
| CORE（闽/京/青/辽） | **+9.4%** | **+4.5%** |
| HOLDOUT（12 图） | **+3.2%** | **+2.6%** |
| 全部 16 场景 | **+4.7%** | **+3.1%** |

相对 `031458`（+4.4 / +2.8）：全场 **+0.3**，holdout **+0.4**。主要增益：台 **+11.6→+14.9**、川 **+5.4→+6.8**、辽 **+7.4→+8.1**；晋略 **-0.15**（可接受）。青 `save_agl` 持平、`save_best≈0`（`preferred > clearance+1.2` 时清 prior）。  
手段：巡航带安全侧向探测 + **真值门控走廊提交** + **双同意 1-via 蒸馏**（≤350 m、早期稀疏刷新）。曾试「`guide_via` 锁定后清 prior」：晋回到基线，但台/川/辽增益全灭，已撤回。
...
| liaoning_coast | **+8.1** | **+8.1** | 蒸馏 + 真值门（原 +7.4） |
| xinjiang_gobi | 0.0 | 0.0 | 干旱开阔 |
| sichuan_foothills | **+6.8** | **+6.8** | 真值门放过真赢走廊（原 +5.4） |
| shanxi_loess | **+6.5** | **+6.5** | 略逊原 +6.7；远 via / 清 prior 会杀台川辽 |
| taiwan_hills | **+14.9** | **+14.9** | 本轮最大增益（原 +11.6） |
...
| jilin_forest | **+2.0** | **+0.5** | 微层 1.033（原 save_best -1.4） |
...
均值 `gap_stf` ≈ **+0.1%**。剩余正缝主要在 **晋/吉/滇**（1-via 真值走廊），不是缺 2-via 几何。STF 与执行层同样能量门控。

### 仍未解决（按优先级）

1. 晋/吉 live 几乎贴直线，而真值信念能生成更优 1-via——缝在 **观测覆盖 / 终选 Joules**，再砸门控收益小  
2. 难图仍大缝时再评估残差 ML；热盘旋在真值能量下仍多为负  
...
1. 晋 −0.15 与台/川/辽增益绑死：清 prior / 收紧 Joules 会修晋但抹增益；优先找「只伤晋」的 prior 竞争条件，勿再盲加总开关  
2. 吉/滇开环缝（`gap_stf`）：观测覆盖 / 终选 Joules，再砸门控收益小  
3. 真机无 truth 时：用「强信念边 + 不确定度」代理真值门；蒸馏改为离线 prior 表  
4. 否则转向真机噪声 / 日志回放；ML 仍低优先
