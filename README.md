...
### 最新四场景结果（`multi-scenario-energy-20260808-204619`）

| 场景 | vs 名义直线 | vs 最佳高度带 | z_max | 说明 |
|------|-------------|---------------|-------|------|
| fujian_hills | 0% | 0% | 1.01 | 真值信念下走廊可省电；在线 ML 信念尚未稳定兑现 |
| beijing_plain | 0% | 0% | 1.00 | 贴名义直线 |
| qinghai_ridge | **+18.3%** | -1.7% | 2.70 | 主要靠选对更高巡航带 |
| qingdao_coast | **0%** | 0% | 1.07 | 无劣质绕行；高度稳定 |
| 均值 | +4.6% | -0.4% | — | 无单场景大亏；相对最佳带仍接近持平 |

对比：修复前糟糕 run `...-104637` 均值 **-29%**（错误高层）。  
中间曾出现「走廊过松 → 福建 +3.5% 但青岛 -8.9%」——已用「必须明确更省电」规则收回，避免单图过拟合。

### 仍未解决

1. **在线走廊收益未稳定兑现** — 单元测试/真值信念下走廊可胜出，但 ML 信念常抹平侧向风差  
2. **`save_best%` 未稳定为正** — 青海略逊于最优常数 AGL  
3. **信念每步预测仍整场覆盖观测** — 在线感知积累有限，是走廊难兑现的主因之一  
