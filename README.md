...
- **强风平坦纪律（山东类）**：航迹 DEM 上升不足一个高度层且路径 ambient ≥3 m/s 时，(1) 禁 soft prior / 跳过 prior 蒸馏；(2) **不生成侧向走廊**；(3) **巡航带封顶 clearance+0.15**（开环 aloft 可省电，闭环爬到 z=2 曾 −20%）。闽类弱风平坦（~2 m/s）与有起伏图（辽/晋/台/青）不受影响。
- **动态 via 重竞争（重规划）**：
...
### 最新 20 场景结果（`multi-scenario-energy-20260810-202627`）

| 分组 | vs 名义直线 | vs 最佳高度带 |
|------|-------------|---------------|
| CORE（闽/京/青/辽） | **+10.1%** | **+5.2%** |
| HOLDOUT（16 图，含 4 新） | **+2.5%** | **+2.0%** |
| 全部 20 场景 | **+4.0%** | **+2.6%** |

相对 `162059`（16 图 ALL **+5.0** / CORE **+10.1** / HOLDOUT **+3.3**）：旧 16 图逐图持平；4 张新 holdout（浙/鲁/渝/桂）≈0 稀释总均值。  
山东此前 **−20%**（走廊+爬升 thrash）→ **≈0**（强风平坦纪律）。

手段：soft prior 真值 ambient / Joules / 侧向边；**home→goal** DEM 起伏（非中途剩余航段）；强风平坦禁走廊并封顶巡航带。

| 场景 | save_agl% | save_best% | 备注 |
|------|-----------|------------|------|
| fujian_hills | **+11.2** | **+11.2** | 弱风平坦，走廊保留 |
| beijing_plain | 0.0 | 0.0 | 平地对照 |
| qinghai_ridge | **+19.6** | **+0.0** | 持平 |
| liaoning_coast | **+9.4** | **+9.4** | 持平 |
| xinjiang_gobi | 0.0 | 0.0 | 干旱开阔 |
| sichuan_foothills | **+6.8** | **+6.8** | 持平 |
| shanxi_loess | **+8.1** | **+8.1** | 持平 |
| taiwan_hills | **+14.9** | **+14.9** | 持平 |
| gansu_hexi | 0.0 | 0.0 | 真值边挡住假 soft |
| guizhou_karst | **+0.2** | **+0.2** | 持平 |
| hainan_coast | **+3.6** | **+0.1** | 持平 |
| hubei_jianghan | **+1.1** | **+1.1** | 持平 |
| tibet_lhasa | 0.0 | 0.0 | 河谷对照 |
| jilin_forest | **+2.0** | **+0.5** | 开环 via 多爬升；live 放行会亏，仍留缝 |
| neimeng_grass | 0.0 | 0.0 | 草原对照 |
| yunnan_karst | **+2.9** | **+1.6** | 持平 |
| zhejiang_hills | 0.0 | 0.0 | 新 holdout |
| shandong_taishan | 0.0 | 0.0 | 新；曾 −20%，强风平坦纪律 |
| chongqing_hills | 0.0 | −1.1 | 新 holdout |
| guangxi_guilin | 0.0 | 0.0 | 新 holdout |
...
| D. 新 holdout 稀释均值 | **已计入** | 20 图 ALL +4.0；鲁 −20→0；浙/渝/桂 ≈0 |
| E. 强风平坦假赢 | **已收敛** | 禁走廊 + 巡航封顶；勿再开环 aloft 当闭环收益 |

### 开环上界

均值 `gap_stf` ≈ **+0.1%**（相对旧 closed）。晋/闽 live 已再抬；剩余正缝主要在 **吉/滇**。吉林开环 via 多 DEM 爬升，live 放行会亏——缝在开环/闭环不一致，不是缺门控。

### 仍未解决（按优先级）

1. **吉林**：开环 1-via 省电但多爬升；现保留爬升预筛。需执行层/STF 或更贴地 via，勿再盲放 soft  
2. 滇小缝；真机无 truth 代理门 / 离线 prior 表  
...
1. 吉林：开环/闭环 DEM 不一致——试更贴地 via 偏移或执行跟踪，勿放爬升预筛  
2. 真机无 truth：信念边+不确定度代理真值门；离线 prior 表  
3. 否则真机噪声 / 日志回放；ML 仍低优先
