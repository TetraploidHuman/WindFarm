...
- **信念预测融合**：与先验滤波而非整场覆盖；**观测过的剪切/抬升叶**在与平滑预报不一致时额外降 Kalman 增益，减轻边被冲掉
- 近期观测与侧向风差可保留
...
### 最新 16 场景结果（`multi-scenario-energy-20260810-002107`）
...
扩编后琼/吉高度缝已用通用门控兑现；川/晋/台仍可能有 1～3% 走廊缝。需要时重跑 `scripts/eval_open_loop_upper_bound.py`（STF 已与执行层同样能量门控）。

### 仍未解决（按优先级）

1. 川/晋/台等走廊开环缝（信念边兑现，勿过拟合）  
2. 难图 headroom 清楚后再评估残差 ML；热盘旋在真值能量下仍多为负  
...
1. 针对剩余开环缝（川/晋/台）做信念–导引一致性，不抬高京/疆/甘误走廊率  
2. 难图仍大缝时再加强残差 ML  
