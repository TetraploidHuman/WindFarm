...
- **防过拟合**：不按地图写特例。曾试「沿航迹顺风边」解锁台湾，`path_model` 变差已撤回；现用通用地形/几何 + 微风垂直门控

### 最新 8 场景结果（`multi-scenario-energy-20260809-220038`）
...
| taiwan_hills | **+8.2** | **+8.2** | 几何走廊兑现 |

对比：四川 **0 → +1.8%**；台湾保持 **+8.2%**；HOLDOUT **+3.7% → +4.2%**；8 场景均值 **+5.3% → +5.5%**。

### 负收益 / 权衡（当前）

| 类型 | 状态 | 说明 |
|------|------|------|
| A. 弱风误走廊 | **已收敛** | 硬地板 + 几何/微风垂直双门；四川转正，北京/新疆仍 0 |
...
1. **盘旋热利用仍弱**  
2. **微风垂直走廊**仍依赖信念 `w`；门槛过松曾短暂误绕（四川 −0.1%） 
### 评测怎么读
