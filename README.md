...
多场景能量评测（`scripts/eval_multi_scenario_energy.py`）显示：系统经常只做到「选一条常数离地高度的直线」，相对最优直线基线平均省电接近 **0%**，选错高度时甚至亏 **20%～50%+**。表面像「会利用风」，实际很少真正绕行、盘旋或跟随上升气流。

### 核心原因（按严重度）

1. **终选被直线导引垄断**（`planner.plan_path`）  
   非直线路径能量只要高于首选直线的约 `1.001` 倍就被丢弃；走廊还要再低于 `corridor_energy_margin`（默认 `0.97`）；MPC / 错高度带会被 snap 回直线。结果几乎总是 `guide_straight_agl_*`。

2. **巡航高度 sticky 易爬错层**（`_select_preferred_cruise_band`）  
   升层门槛约 `1.5%`（`climb_need=0.985`），降层更严（`dump_need=0.975`），每步最多爬 `+0.35` 带。噪声下会逐步锁到过高 AGL，电量大亏。

3. **上升气流在气动模型里太弱**（`controller.aerodynamic_power_w`）  
   `thermal_credit` 要求 `w > sink≈0.58` 才生效；`uplift_credit` 仅约 `17.4·w` W。弱/中等抬升几乎排不动候选路径。

4. **安全项惩罚 `|w|`**（`belief.apply_prediction`）  
   `safety_penalty` 含 `0.20·|wind_w|/2.5`，上升区反而更「危险」，抵消垂直收益。

5. **MPC 盘旋被反向惩罚**（`_score_continuous_proposals`）  
   水平前进 `<0.20` 时，对抬升扣分并把信念能量奖励乘 `0.15`，热盘旋很难胜出。

6. **信念能量定义不准**  
   `expected_energy_gain` 用 `0.16·水平风速`（不区分顺/逆风），易把强逆风当「有收益」。

7. **规划与扣电/评测口径不一致**  
   执行与评测有 `E *= 1 - 0.20·max(w,0)` 折扣，规划内部 `transition_energy_j` 无此项。

### 现有评测怎么读

| 结果类型 | 含义 |
|---------|------|
| `save_agl% ≈ 0` 且 `z_max≈1.0` | 完全贴合名义直线，无额外风场收益 |
| `save_agl%` 高但 `save_best%≈0` | 主要是选对了更高巡航带，不是动态寻风 |
| `save_*` 大幅为负且 `z_max≈3` | 高度策略失控，爬错层 |

示例：`runs/multi-scenario-energy-20260808-132500` 均值 vs 名义 +4.4%、vs 最佳带 **-0.3%**；`...-104637` 均值 **-29%**（错误高层）。

### 建议修复优先级

1. 放宽终选/走廊门限，停止无条件把 MPC snap 成直线  
2. 对称化升/降层证据，限制 sticky 累计爬升并定期重估 clearance  
3. 降低抬升门槛、加大 uplift 信用，统一规划与评测能量公式  
4. 安全只罚下沉/切变，不罚上升侧 `|w|`；能量收益改为顺风分量 + `max(w,0)`  
5. 对 `thermal_orbit` 免除「低前进罚抬升」  
6. 信念风场改为滤波融合，避免每步预测整场覆盖观测  

## 后续方向

- 修复上述风场利用链路后，用多场景能量评测验证 `save_best%` 稳定为正
- 在生产环境中优先接入 XGBoost / LightGBM
- 接入真实 DEM、地表覆盖与机载遥测日志
- 从单机信念更新扩展到多机协同信念共享
- 评测增加消融基线（仅水平风 / 禁抬升 / 最优离线开环），避免「贴直线 = 0%」掩盖真实能力
